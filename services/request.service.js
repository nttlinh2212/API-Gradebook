...
const requestService = {
    ...
    async findRequestsOfAStudent(userId) {
        ...
        for (let i = 0; i < list.length; i++) {
            const gradeComp = await classService.findOneGradeFinalize(
                list[i].class._id,
                list[i].gradeIdentity
            );
    ...
    async findRequestsOfATeacher(userId) {
        ...
        let ret = [];
        for (let i = 0; i < list.length; i++) {
            let gradeComp = await classService.findOneGradeFinalize(
                list[i].class._id,
                list[i].gradeIdentity
            );
            if (!gradeComp || gradeComp.gradeStructure.length === 0) {
    ...
    async add(reqObj) {
        const studentId = (await userService.findById(reqObj.student))
            .studentId;
        const composition = await gradeService.findStudentComposition(
            studentId,
