...
const inviteEmailSchema = JSON.parse(
    await readFile(
        new URL('../form-schemas/invite-email.json', import.meta.url)
    )
);
const tokenSchema = JSON.parse(
    await readFile(new URL('../form-schemas/token.json', import.meta.url))
);
const gradeStructureSchema = JSON.parse(
    await readFile(
        new URL('../form-schemas/grade-structure.json', import.meta.url)
    )
);
const listStudentsSchema = JSON.parse(
    await readFile(
        new URL('../form-schemas/list-students.json', import.meta.url)
    )
);
const studentGradesSchema = JSON.parse(
    await readFile(
        new URL('../form-schemas/student-grades.json', import.meta.url)
    )
);
const cellGradesBoardSchema = JSON.parse(
    await readFile(
        new URL('../form-schemas/cell-grades-board.json', import.meta.url)
    )
);
...
router.get(
    ...
    async function (req, res) {
        ...
        const classObj = await classService.findDetailClassInfoByIdWithRole(
            id,
            req.role
        );
        if (!classObj) {
...
router.get(
    ...
    async function (req, res) {
        const id = req.params.id || 0;
        const listStudents = await classMemberService.findAllStudentsInAClass(
            id
        );
        const listTeachers = await classMemberService.findAllTeachersInAClass(
            id
        );
        const retJson = {
            _id: id,
    ...
    async function (req, res) {
        ...
        for (let i = 0; i < structure.gradeStructure.length; i++) {
            ...
            if (!structure.gradeStructure[i].identity) {
                structure.gradeStructure[i].identity =
                    new mongoose.Types.ObjectId(); //randomstring.generate(10);
            }
        }
        if (sum > 10) {
            return res.status(400).json({
        ...
        const ret = await classService.patch(classId, {
            gradeStructure: structure.gradeStructure,
        });
...
router.get(
    ...
    async function (req, res) {
        const classId = req.params.id || 0;
        const ret = await classService.findByIdHavingSelect(classId, {
            gradeStructure: 1,
        });
        //console.log("RESULT OF GRADE STRUCTURE:",ret);
...
router.delete(
    ...
    async function (req, res) {
        ...
        //return
        const retJson = await classService.findByIdHavingSelect(classId, {
            gradeStructure: 1,
        });
        res.status(200).json(retJson);
    }
...
router.post(
    ...
    async function (req, res) {
        ...
        if (!listStudents) return res.status(200).json(retJson);
        for (const s of listStudents) {
            const element = await notiService.addNewFinalize(
                s.user._id,
                classId,
                req.className,
                req.userId
...
router.post(
    ...
    async function (req, res) {
        ...
        var valueArr = listStudents.map(function (item) {
    ...
    async function (req, res) {
        ...
        for (const s of grades) {
            ...
            try {
                ...
                if (isNaN(grade)) throw new Error('Grade is not number');
                if (grade > 10 || grade < 0)
        ...
        if (!check) {
            return res.status(400).json({
                message: 'Grade Composition do not exist in Grade Structure.',
            });
        }
        ...
        var valueArr = grades.map(function (item) {
            return item.studentId;
        });
        var isDuplicate = valueArr.some(function (item, idx) {
            return valueArr.indexOf(item) != idx;
        ...
        for (const s of grades) {
            const exist = await classService.findStudentIdInListStudents(
                classId,
                s.studentId
            );
        ...
        for (const s of grades) {
            ...
            const one = await gradeService.addIfNotExistElseUpdate(
                condition,
                newInfo
            );
            ret.push(one);
...
router.patch(
    '/:id/cell-grades-board',
    validate(cellGradesBoardSchema),
    authMdw.auth,
    authMdw.authMember,
    authMdw.authTeacher,
    authMdw.class,
    async function (req, res) {
        ...
        const exist1 = await classService.findStudentIdInListStudents(
            classId,
            studentId
        );
