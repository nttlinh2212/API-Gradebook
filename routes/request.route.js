...
const commentSchema = JSON.parse(
    await readFile(new URL('../form-schemas/comment.json', import.meta.url))
);
const finalDecisionSchema = JSON.parse(
    await readFile(
        new URL('../form-schemas/final-decision.json', import.meta.url)
    )
);
const requestSchema = JSON.parse(
    await readFile(new URL('../form-schemas/request.json', import.meta.url))
);
...
router.post(
    ...
    async function (req, res) {
        ...
        try {
            const isJoin = await classMemberService.findARoleInAClass(
                req.userId,
                req.body.class,
                'student'
            );
            const isComposition = await classService.findOneGradeFinalize(
                req.body.class,
                req.body.gradeIdentity
            );
            if (
                !isJoin ||
                !isComposition ||
        ...
        if (exist) {
            return res.status(400).json({
                message:
                    'You only have one chance to request a review for a grade compostion.',
            });
        }
        ...
        for (const t of listTeachers) {
            await notiService.addNewRequest(
                t.user._id,
                classInfo.name,
                ret._id,
                req.userId
...
router.patch(
    ...
    async function (req, res) {
        ...
        const studentId = (await userService.findById(req.request.student))
            .studentId;
        ...
        await notiService.addNewDecision(
            req.request.student,
            teacherName,
            id,
            req.userId
