...
const schema = JSON.parse(
    await readFile(new URL('../form-schemas/login.json', import.meta.url))
);
const emailSchema = JSON.parse(
    await readFile(new URL('../form-schemas/email.json', import.meta.url))
);
const tokenSchema = JSON.parse(
    await readFile(new URL('../form-schemas/token.json', import.meta.url))
);
const accessTokenSchema = JSON.parse(
    await readFile(
        new URL('../form-schemas/access-token.json', import.meta.url)
    )
);
const resetPassSchema = JSON.parse(
    await readFile(new URL('../form-schemas/reset-pw.json', import.meta.url))
);
const rfSchema = JSON.parse(
    await readFile(new URL('../form-schemas/rf.json', import.meta.url))
);
    ...
    async function (req, res) {
        ...
        const msg = {
            ...
            html: renderContentEmail(
                'Verify email',
                'Please click the button below to verify this email.',
                'Verify email',
                '10 mins',
                link
            ),
...
router.post('/reset-pw/send', validate(emailSchema), async function (req, res) {
    ...
    //send email
    ///class/join/6192342f1da8dc83c060b2a0?token=jllaGPGE&role=teacher
    const link = `${urlFE}forgot-password?email=${encodeURIComponent(
        email
    )}&token=${token}`;
    sgMail.setApiKey(SENDGRID_API_KEY);
    const msg = {
