...
const profileSchema = JSON.parse(
    await readFile(new URL('../form-schemas/profile.json', import.meta.url))
);
const passSchema = JSON.parse(
    await readFile(new URL('../form-schemas/password.json', import.meta.url))
);
...
router.patch(
    ...
    async function (req, res) {
        ...
        if (!user.password) {
            return res.status(400).json({
                message:
                    'Current password does not exist. Please click on forget password in the login page to create a password!',
            });
        }
